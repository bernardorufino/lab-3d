...
void LoadBezier(char *file_name, point3D_type** ap, GLint* np, GLint* mp)
{
	FILE *file;
...
void BezierRecursiveSubdivision(point3D_type a[], GLint n, GLint m, int depth=0)
{  
	if (depth > 9 || Limit(a, n, m, repeat(' ', depth))) 
	{
	...
	{ 
		...
		point3D_type* controls[] = { b, c };
		for (int i = 0; i < sizeof(controls) / sizeof(point3D_type*); i++) 
		{
			BezierRecursiveSubdivision(controls[i], n, m, depth + 1);
		}
	}
}
